...
float distanceFromLineCropped(float2 origin, float angle, float2 plot, float plotAngle)
{
    ...
    float pastOriginMask = abs(mod(angle - plotAngle + 3.1415926, 6.2831853) - 3.1415926) < 1.5707;
    float dist = pastOriginMask * distanceFromLine(origin, angle, plot);
    
    //If we are behind the line, just give the distance between the start point and the plot
    dist += (1 - pastOriginMask) * length(origin - plot);
...
float4 main(float2 uv : TEXCOORD) : COLOR
{
    ...
    circleMask *= distanceFromLine <= halfLaserWidth + bloomSize;
    ...
    //The part that's outside the bloom
    float bloomMask = distanceFromLine > halfLaserWidth;
    float4 returnColor = (1 - bloomMask) * float4(laserColor * laserOpacity, laserOpacity);
    ...
    float3 bloomColor = lerp(float3(0, 0, 0), laserColor, bloomBlendFactor);
    float opacity = lerp(0, laserOpacity, bloomBlendFactor) * mainOpacity * bloomMaxOpacity;
    bloomColor = bloomColor * opacity;
    
    returnColor += tex2D(Texture1Sampler, uv) + float4(bloomColor, opacity) * bloomMask; //Have to sample texture1 because if we don't it wont use the correct sampler for the noise
    return returnColor * circleMask;
}

technique Technique1
{
    pass PixelatedSightLinePass
    {
