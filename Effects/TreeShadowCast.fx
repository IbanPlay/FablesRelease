...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float opacity = pow(uv.y, 1.5);
    //Fade at the horizontal edges of the quad
    opacity *= pow(invlerp(0.5, 0.45, abs(uv.y - 0.5)), 0.6);
    ...
    float3 shadowTint = lerp(float3(0.08, 0.16, 0.46), float3(0.33, 0.1, 0.1), invlerp(0.4, 0.5, abs(uv.y - 0.5)));

    shadowColor += float3(0.08, 0.16, 0.46) * colorfulness;
