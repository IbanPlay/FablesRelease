...
float4 Recolor(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    color.g += underGlow * 0.4;
    color.b += underGlow * 1.3;
    
    //Add spores that slide throught the effect
    float noiseValue = GetNoiseStrenght(noiseCoords + float2(0.3, uTime * 0.1)) + GetNoiseStrenght(noiseCoords + float2(0.7 + uTime * 0.025, uTime * 0.15 - 0.5));
    color.rg += noiseValue * 2.5;
    color.b += noiseValue * 8;
    
    //Make the spores float up
    if (originalAlpha == 0 && noiseValue > 0.1)
        originalAlpha += (noiseValue * min(1, frameY * 2)) * originalAlpha;
