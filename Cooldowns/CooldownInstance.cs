...
{
    [Serializable]
    public class CooldownInstance : ISerializable
    {
        ...
        internal CooldownInstance(Player p, string id, TagCompound tag)
        {
            ...
            Cooldown cd = CooldownLoader.Get(id);
            if (cd == null)
            {
                player = null;
                return;
            ...
            if (netID != registeredNetID)
            {
                ...
                CalamityFables.Instance.Logger.Warn($"Cooldown \"{id}\" loaded from NBT with discrepant netID {netID}. This cooldown was registered with netID {registeredNetID}");
        ...
        */
        ...
        public CooldownInstance(SerializationInfo info, StreamingContext ctxt)
        {
            netID = (ushort)info.GetValue("netID", typeof(ushort));
            player = Main.player[(ushort)info.GetValue("playerIDByte", typeof(byte))];
            duration = (int)info.GetValue("duration", typeof(int));
            timeLeft = (int)info.GetValue("timeLeft", typeof(int));

            string id = CooldownLoader.registry[netID].ID;
            AssignHandler(CooldownLoader.Get(id));
        }

        public void GetObjectData(SerializationInfo info, StreamingContext ctxt)
        {
            info.AddValue("netID", netID);
            info.AddValue("playerIDByte", (byte)player.whoAmI);
            info.AddValue("duration", duration);
            info.AddValue("timeLeft", timeLeft);
        }

        public CooldownInstance(ushort netId, int player, int duration, int timeLeft)
        {
            netID = netId;
