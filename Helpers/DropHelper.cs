...
// This is taken from the original CalamityMod's DropHelper, written by Ozzatron
// ^ Tmod really should have all of this built in...
namespace CalamityFables.Helpers
{
    ...
    public static class DropHelper
    {
        ...
        public static void FablesifyDropPool(this ILoot loot)
        {
            var rules = loot.Get();
            for (int i = rules.Count - 1; i >= 0; i--)
            {
                if (!rules[i].FablesifyDropRule())
                    rules[i] = new DropNothing();
            }
        }

        private static bool FablesifyDropRule(this IItemDropRule rule)
        {
            if (rule is CommonDrop drop)
            {
                if (ParasiteCoreSystem.Items_CalToFables.TryGetValue(drop.itemId, out int fablesID))
                    drop.itemId = fablesID;
                else if (ParasiteCoreSystem.RemovedCalItems.Contains(drop.itemId))
                    return false;
                return true;
            }
            else if (rule is ItemDropWithConditionRule conditionalDrop)
            {
                if (ParasiteCoreSystem.Items_CalToFables.TryGetValue(conditionalDrop.itemId, out int fablesID))
                    conditionalDrop.itemId = fablesID;
                else if (ParasiteCoreSystem.RemovedCalItems.Contains(conditionalDrop.itemId))
                    return false;
                return true;
            }
            else if (rule is DropBasedOnExpertMode expertDrop)
            {
                if (!expertDrop.ruleForNormalMode.FablesifyDropRule())
                    expertDrop.ruleForNormalMode = new DropNothing();
                if (!expertDrop.ruleForExpertMode.FablesifyDropRule())
            ...
            {
                if (!masterDrop.ruleForDefault.FablesifyDropRule())
                    masterDrop.ruleForDefault = new DropNothing();
                if (!masterDrop.ruleForMasterMode.FablesifyDropRule())
            ...
            else if (rule is LeadingConditionRule lcr)
            {
                ...
                {
                    if (!lcr.ChainedRules[i].RuleToChain.FablesifyDropRule())
